...
module.exports = function (content, options) {
	...
	parse(content, options).forEach(function (tag) {
		if (!_.has(options.processors, tag.name) || !_.isFunction(options.processors[tag.name])) {
			throw new Error('undefined processor "' + tag.name + '".');
		}

		var macrosResult = options.processors[tag.name].call(options, tag.params, tag.inner, tag.source);
